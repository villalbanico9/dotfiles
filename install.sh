...
sudo pacman -S adwaita-cursors alsa-lib alsa-plugins alsa-tools alsa-utils apr-util arp-scan autoconf automake base base-devel bat bspwm burpsuite bzip2 clang cmake crackmapexec curl devtools docker dpkg exploitdb feh firefox flameshot fuse fzf gc gcc gdb ghidra git gnu-netcat go gobuster gzip hashcat hash-identifier hydra inetutils impacket iputils john kitty kvantum less lib32-gcc-libs lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings lightdm-webkit2-greeter locate lsd lua lxappearance macchanger man-db mariadb mesa mesa-utils metasploit net-tools networkmanager nm-connection-editor nmap openvpn papirus-icon-theme paru perl picom pocl polybar proxychains psmisc pulseaudio python python2 qt6ct responder rlwrap rofi ruby sqlite samba sqlmap starship sxhkd systemd tar tcpdump thunar tree unzip util-linux webkit2gtk wfuzz wget wireshark-qt wmname xclip xdg-utils xdotool xf86-input-libinput xorg-server-common xorg-xrandr xterm yay zip zsh zsh-autosuggestions zsh-syntax-highlighting --noconfirm --needed
